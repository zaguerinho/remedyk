...
class CheckActivation extends Component{
	public function init() {
		if (! Yii::$app->user->isGuest){
			...
			if ($user->status == User::STATUS_TO_CONFIRM && Yii::$app->request->url != '/site/activation-pending' && Yii::$app->request->url != '/site/logout'){
			...
			if ($user->profileType() == User::PATIENT) {
				return Yii::$app->response->redirect(Yii::$app->params['patientsDomain']);
			}
