...
use doctors\models\search\DoctorPaymentSearch;
...
class DoctorPaymentsController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if($request->isAjax){
            ...
            */
            ...
            if($request->isGet){
                return [
                    'title'=> "Create new DoctorPayment",
                ...
                return [
                    ...
                    'title'=> "Create new DoctorPayment",
                    'content'=>'<span class="text-success">Create DoctorPayment success</span>',
                    'footer'=> Html::button('Close',['class'=>'btn btn-default pull-left','data-dismiss'=>"modal"]).
    ...
    public function actionAjaxUploadInvoice(){

    	Yii::$app->response->format = Response::FORMAT_JSON;
    	if (Yii::$app->request->isPost){
    		$post = Yii::$app->request->post();

    		$doctorPayment = DoctorPayment::findOne($post['doctorPaymentId']);
    		$doctorPayment->invoice_name = $post['fileName'];
    		$doctorPayment->invoiceBase64data = $post['fileContent'];
    		$doctorPayment->status = DoctorPayment::STATUS_PENDING_PAYMENT;

    		if ($doctorPayment->save()){
    			return ['status' => 200];
    		}
    		return ['status' => 500];
    	}
